...
static std::unique_ptr<Atom> build_data_atom(const std::vector<uint8_t> &bytes,
                                             uint32_t type = 1) {
...
static std::unique_ptr<Atom> build_string_item(const char *tag,
...
build_cover_item(const std::vector<uint8_t> &cover) {
...
std::unique_ptr<Atom> build_meta_atom(const MetadataSet &meta) {
  ...
  if (!meta.title.empty())
    ilst->add(build_string_item("\xA9"
                                "nam",
                                meta.title));
