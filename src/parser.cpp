...
static std::vector<uint8_t> scan_ilst_payload(const std::string &path) {
  ...
  for (size_t i = 0; i + 4 <= data.size(); ++i) {
    if (data[i] == 'i' && data[i + 1] == 'l' && data[i + 2] == 's' &&
        data[i + 3] == 't') {
      if (i < 4)
        continue;
      uint32_t size = (data[i - 4] << 24) | (data[i - 3] << 16) |
                      (data[i - 2] << 8) | (data[i - 1]);
      if (size < 8)
        continue;
...
static void parse_mdhd(std::istream &in, uint64_t size, uint32_t &timescale,
                       uint64_t &duration) {
  ...
  uint64_t consumed =
      (version == 1 ? 1 + 3 + 8 + 8 + 4 + 8 : 1 + 3 + 4 + 4 + 4 + 4);
  if (size > consumed)
...
static void parse_meta(std::istream &in, uint64_t size, ParsedMp4 &out) {
  ...
  auto parse_children = [&](bool consume_reserved) {
    ...
    if (child.type == fourcc("ilst")) {
      ...
      std::cerr << "[parser] captured ilst payload, bytes=" << out.ilst_payload.size() << "\n";
    } else {
...
ParsedMp4 parse_mp4(const std::string &path) {
  ...
  while (in.peek() != EOF) {
    ...
    case ('m' << 24 | 'o' << 16 | 'o' << 8 | 'v'): { // moov
      ...
      while (((uint64_t)in.tellg()) + 8 <= end) {
        ...
        std::cerr << "[parser] moov child=" << std::hex << child.type << std::dec
                  << " size=" << child.size << "\n";
          ...
          // find meta inside udta
          {
            ...
            while (((uint64_t)in.tellg()) + 8 <= udta_end) {
              ...
              if (u.type == ('m' << 24 | 'e' << 16 | 't' << 8 | 'a')) {
                std::cerr << "[parser] found meta inside udta\n";
        ...
        case ('m' << 24 | 'e' << 16 | 't' << 8 | 'a'): {
          // Some files place meta directly under moov (not inside udta)
          std::cerr << "[parser] found meta under moov\n";
        ...
        case ('t' << 24 | 'r' << 16 | 'a' << 8 | 'k'): {
          ...
          while (trak_remain >= 8) {
            ...
            if (tchild.type == ('m' << 24 | 'd' << 16 | 'i' << 8 | 'a')) {
              std::cerr << "[parser] trak: entering mdia\n";
              // parse mdia
              ...
              while (mdia_remain >= 8) {
                ...
                if (m.type == ('m' << 24 | 'd' << 16 | 'h' << 8 | 'd')) {
                  ...
                  std::cerr << "[parser] hdlr=" << std::hex << handler_type
                            << std::dec << "\n";
  ...
  if (out.stsz.empty() || out.stco.empty() || out.stsc.empty() ||
      out.stsd.empty()) {
    std::cerr << "[parser] fallback flat scan for stbl atoms\n";
    ...
    auto grab_atom = [&](const char *fourcc, std::vector<uint8_t> &dst) {
      ...
      for (size_t i = 0; i + 8 <= buf.size(); ++i) {
        if (buf[i + 4] == static_cast<uint8_t>(fourcc[0]) &&
          ...
          uint32_t sz = (buf[i] << 24) | (buf[i + 1] << 16) |
                        (buf[i + 2] << 8) | buf[i + 3];
          if (sz >= 8 && i + sz <= buf.size()) {
            dst.assign(buf.begin() + i + 8, buf.begin() + i + sz);
            std::cerr << "[parser] grabbed " << fourcc
                      << " via raw scan, bytes=" << dst.size() << "\n";
            break;
          }
        }
      }
...
if (out.ilst_payload.empty()) {
  ...
  if (!ilst.empty()) {
    ...
    std::cerr << "[parser] ilst found via naive scan, bytes="
              << out.ilst_payload.size() << "\n";
  }
}
  return out;
}
