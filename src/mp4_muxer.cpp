...
#include "mdat_writer.hpp"  // our write_mdat + patch helpers from previous step
...
// -----------------------------------------------------------------------------
static bool parse_jpeg_dimensions(const std::vector<uint8_t> &data,
...
bool write_mp4(const std::string &output_path, const AacExtractResult &aac,
               ...
               Mp4aConfig audio_cfg, const MetadataSet &metadata,
               bool fast_start, const std::vector<uint8_t> *ilst_payload) {
  //
  // 0) open file
  //
  ...
  static const uint8_t ftyp_box[] = {
      0x00, 0x00, 0x00, 0x24, 'f', 't', 'y', 'p', 'M', '4', 'V', ' ',
      0x00, 0x00, 0x00, 0x01, 'm', 'p', '4', '2', 'i', 's', 'o', 'm',
      'M',  '4',  'A',  ' ',  'M', '4', 'V', ' ', 'd', 'b', 'y', '1'};
  ...
  audio_cfg.sample_rate =
      aac.sample_rate ? aac.sample_rate : audio_cfg.sample_rate;
  audio_cfg.channel_count =
      aac.channel_config ? aac.channel_config : audio_cfg.channel_count;
  audio_cfg.sampling_index =
      aac.sampling_index ? aac.sampling_index : audio_cfg.sampling_index;
  audio_cfg.audio_object_type = aac.audio_object_type
  ...
  std::vector<uint32_t> audio_chunk_plan =
      aac.stsc_payload.empty()
          ? build_audio_chunk_plan(audio_sample_count)
  ...
  if (!aac.stsd_payload.empty() && !aac.stts_payload.empty() &&
      !aac.stsc_payload.empty() && !aac.stsz_payload.empty() &&
      !aac.stco_payload.empty()) {
    // Golden-aligned path: reuse the source audio stbl verbatim. Rebuilding
    // stbl for audio triggered decoding issues in Apple players even when the
    // fields were “correct” per spec, so we preserve the original structure
    // whenever we can.
    std::cerr << "Reusing source audio stbl\n";
    stbl_audio = build_audio_stbl_raw(aac.stsd_payload, aac.stts_payload,
                                      aac.stsc_payload, aac.stsz_payload,
                                      aac.stco_payload);
  } else {
    std::cerr << "Building new audio stbl\n";
    stbl_audio = build_audio_stbl(audio_cfg, aac.sizes, audio_chunk_plan,
                                  audio_sample_count, nullptr);
  }
  ...
  if (has_image_track) {
    stbl_image = build_image_stbl(image_chapters, chapter_timescale,
                                  image_width, image_height, image_chunk_plan);
  }
  ...
  const uint64_t tkhd_audio_duration =
      (audio_duration_ts * mvhd_timescale) / audio_timescale;
  const uint64_t tkhd_chapter_duration =
      (text_duration_ts * mvhd_timescale) / chapter_timescale;
  const uint64_t tkhd_image_duration =
      has_image_track ? (image_duration_ts * mvhd_timescale) / chapter_timescale
                      : 0;
  const uint64_t mvhd_duration =
      has_image_track ? std::max({tkhd_audio_duration, tkhd_chapter_duration,
                                  tkhd_image_duration})
  ...
  auto trak_audio = build_trak_audio(AUDIO_TRACK_ID, audio_timescale,
                                     audio_duration_ts, std::move(stbl_audio),
                                     chapter_refs, tkhd_audio_duration);

  auto trak_text =
      build_trak_text(TEXT_TRACK_ID, chapter_timescale, text_duration_ts,
                      std::move(stbl_text), tkhd_chapter_duration);

  std::unique_ptr<Atom> trak_image;
  if (has_image_track) {
    trak_image = build_trak_image(
        IMAGE_TRACK_ID, chapter_timescale, image_duration_ts,
  ...
  //
  auto moov =
      build_moov(mvhd_timescale, mvhd_duration, std::move(trak_audio),
  ...
  if (fast_start) {
    ...
    uint64_t payload_start = static_cast<uint64_t>(ftyp_size) + moov->size() +
                             8; // +8 for mdat header
    MdatOffsets mdat_offs = compute_mdat_offsets(
        payload_start, audio_samples, text_samples, image_samples,
        audio_chunk_plan, text_chunk_plan, image_chunk_plan);
    patch_all_stco(moov.get(), mdat_offs, true);

    // write moov, then mdat
    moov->write(out);
    write_mdat(out, audio_samples, text_samples, image_samples,
               audio_chunk_plan, text_chunk_plan, image_chunk_plan);
  } else {
    ...
    // Optional leading free box before moov (padding). Size/placement mirrors
    // the golden sample to avoid surprising atom ordering sensitivities.
    {
