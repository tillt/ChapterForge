...
static std::vector<uint8_t> build_audio_specific_config(uint8_t audio_object_type,
    ...
    uint8_t byte1 = ((audio_object_type & 0x1F) << 3) | ((sampling_freq_index & 0x0E) >> 1);

    uint8_t byte2 = ((sampling_freq_index & 0x01) << 7) | ((channel_config & 0x0F) << 3);
...
static std::unique_ptr<Atom> build_esds(const Mp4aConfig &cfg) {
    ...
    auto asc =
        build_audio_specific_config(cfg.audio_object_type, cfg.sampling_index, cfg.channel_config);
    ...
    write_u8(p, 0x05);
    write_descr_len_padded4(p, static_cast<uint8_t>(asc.size()));  // typically 2 bytes
