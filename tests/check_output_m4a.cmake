...
if(NOT MP4DUMP_PATH)
    message(WARNING "mp4dump not found; mp4dump checks will be skipped")
endif()

if(NOT ATOMIC_PARSLEY_PATH)
    message(WARNING "AtomicParsley not found; AtomicParsley checks will be skipped")
endif()
...
if(HAVE_INPUT_M4A)
    ...
    # Atom count comparisons: constrain to the primary audio trak so added chapter tracks
    # (text/images) do not trigger mismatches. We approximate by comparing only the first
    # occurrence counts from mp4dump/AtomicParsley (audio track) and ignore extras.
    if(MP4DUMP_PATH)
        foreach(atom IN LISTS ATOMS_TO_CHECK_STR)
            ...
            # Compare only the first (assumed audio) count; ignore extra tracks present in output.
            ...
            if(NOT input_count STREQUAL output_count)
                message(WARNING "mp4dump atom count mismatch for ${atom}: input=${input_count}, output(audio)=${output_count}")
            endif()
        endforeach()
        message(STATUS "mp4dump atom checks completed")
    else()
        message(STATUS "Skipping mp4dump atom checks (mp4dump missing)")
    endif()
    ...
    if(ATOMIC_PARSLEY_PATH)
        foreach(atom IN LISTS ATOMS_TO_CHECK_STR)
            ...
            string(REGEX REPLACE ".*${atom}=([0-9]+).*" "\\1" output_count "${_out}")
            if(NOT input_count STREQUAL output_count)
                message(WARNING "AtomicParsley atom count mismatch for ${atom}: input=${input_count}, output(audio)=${output_count}")
            endif()
        endforeach()
        message(STATUS "AtomicParsley atom checks completed")
    else()
        message(STATUS "Skipping AtomicParsley atom checks (missing)")
    endif()
...
elseif(_mdat_off_hex MATCHES "not-found|unknown")
    ...
    if(NOT _mdat_match)
        ...
        # mp4dump in some builds omits explicit chunk offsets; if so, skip this guard to avoid false positives.
        if(NOT OUTPUT_MP4DUMP MATCHES "chunk offset")
            message(WARNING "Skipping chunk offset guard: mp4dump output does not list offsets")
            set(_skip_chunk_guard TRUE)
        endif()
    endif()
endif()

if(_skip_chunk_guard)
    message(STATUS "Chunk offset guard skipped")
else()
