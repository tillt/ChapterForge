...
foreach(var IN ITEMS INPUT_AAC INPUT_M4A CHAPTER_JSON OUTPUT_M4A CHAPTERFORGE_BIN)
    if(NOT DEFINED ${var})
        message(FATAL_ERROR "${var} is not set")
    endif()
endforeach()

if(NOT EXISTS "${INPUT_AAC}")
    message(FATAL_ERROR "INPUT_AAC not found: ${INPUT_AAC}")
endif()
if(NOT EXISTS "${INPUT_M4A}")
    message(FATAL_ERROR "INPUT_M4A not found (needed for validation): ${INPUT_M4A}")
endif()
...
set(MUX_ARGS "${INPUT_AAC}" "${CHAPTER_JSON}" "${OUTPUT_M4A}")
if(FASTSTART)
...
if(NOT rv EQUAL 0)
    message(FATAL_ERROR "ChapterForge (AAC input) failed (${rv}):\n${err}\n${out}")
endif()
message(STATUS "ChapterForge OK (AAC input -> ${OUTPUT_M4A})")

# Now run the existing tool checks with the freshly built output; we still pass
# the original input M4A so we can compare metadata/audio properties.
